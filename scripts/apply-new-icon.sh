...
mkdir -p "$ROOT/icons"
MASTER_PNG_DEST="$ROOT/icons/icon-modulo-squares.png"
...
MASTER_SVG_SRC="$ROOT/icons/icon-modulo-squares.svg"
if [ -f "$MASTER_SVG_SRC" ]; then
  echo "Syncing master SVG into web directories..."
  if [ -d "$ROOT/packages/web/public" ]; then
    cp -v "$MASTER_SVG_SRC" "$ROOT/packages/web/public/modulo-icon.svg" || true
  fi
  if [ -d "$ROOT/packages/mobile/web" ]; then
    cp -v "$MASTER_SVG_SRC" "$ROOT/packages/mobile/web/modulo-icon.svg" || true
  fi
fi
